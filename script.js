...
let currentSortMode = 'id'; let isDescending = false; 
window.currentDetailedPokemon = null; 
let currentMoveMethod = 'level-up'; 
let currentTopType = ''; 

// --- A. THE INTERFACE & SIDE MENU LOGIC ---
function closeDrawer() { navDrawer.classList.remove('open'); drawerOverlay.classList.remove('visible'); }
toggleBtn.addEventListener('click', () => { navDrawer.classList.toggle('open'); drawerOverlay.classList.toggle('visible'); });
drawerOverlay.addEventListener('click', closeDrawer);
settingsBtn.addEventListener('click', () => { settingsMenu.style.display = settingsMenu.style.display === 'none' ? 'flex' : 'none'; });
[toggleMega, toggleGmax, toggleRegional, toggleAlt].forEach(checkbox => {
    checkbox.addEventListener('change', () => { liveFilterPokemon(tagInput.value.toLowerCase().trim()); });
});

function switchView(tabId) {
    views.forEach(v => { if(v) v.classList.remove('active'); }); tabBtns.forEach(b => b.classList.remove('active'));
    if(tabId !== 'tab-pokemon-btn') resultsDiv.innerHTML = ''; if(tabId !== 'tab-abilities-btn') abilityResultsDiv.innerHTML = ''; if(tabId !== 'tab-moves-btn') movesResultsDiv.innerHTML = ''; if(tabId !== 'tab-items-btn') itemsResultsDiv.innerHTML = '';
    const activeBtn = document.getElementById(tabId); if(activeBtn) { activeBtn.classList.add('active'); headerTitle.innerText = activeBtn.innerText.replace(/[◓✦⚔🎒⊕🧮]/g, '').trim(); }

    if(tabId === 'tab-pokemon-btn') { document.getElementById('view-pokemon').classList.add('active'); liveFilterPokemon(tagInput.value.toLowerCase().trim()); document.getElementById('fab-container').style.display = 'flex'; } else { document.getElementById('fab-container').style.display = 'none'; }
    if(tabId === 'tab-abilities-btn') { document.getElementById('view-abilities').classList.add('active'); displayAbilities(offlineAbilities); }
...
async function saveToDB(storeName, data, key = 'master_key') { const db = await initDB(); return new Promise((resolve, reject) => { const tx = db.transaction(storeName, 'readwrite'); const store = tx.objectStore(storeName); store.put(data, key); tx.oncomplete = () => resolve(); tx.onerror = () => reject(tx.error); }); }
async function loadFromDB(storeName, key = 'master_key') { const db = await initDB(); return new Promise((resolve, reject) => { const tx = db.transaction(storeName, 'readonly'); const store = tx.objectStore(storeName); const request = store.get(key); request.onsuccess = () => resolve(request.result); request.onerror = () => reject(request.error); }); }

// --- C. LOADERS ---
async function loadOfflineData() { const savedData = await loadFromDB('pokemon'); if (savedData && savedData.length > 0) { offlineDatabase = savedData; statusText.innerText = `Ready (${offlineDatabase.length} PKMN)`; tagInput.disabled = false; populateTopFilters(); buildMasterList(); } else { statusText.innerText = "Offline. Please Sync."; } }
async function loadOfflineAbilities() { const saved = await loadFromDB('abilities'); if (saved && saved.length > 0) { offlineAbilities = saved; abilitySearch.disabled = false; buildMasterList(); } }
async function loadOfflineMoves() { const saved = await loadFromDB('moves'); if (saved && saved.length > 0) { offlineMoves = saved; movesSearch.disabled = false; buildMasterList(); } }
async function loadOfflineItems() { const saved = await loadFromDB('items'); if (saved && saved.length > 0) { offlineItems = saved; itemsSearch.disabled = false; } }
...
function populateTopFilters() {
    ...
    if (typeListEl) { const types = ["normal", "fighting", "flying", "poison", "ground", "rock", "bug", "ghost", "steel", "fire", "water", "grass", "electric", "psychic", "ice", "dragon", "dark", "fairy"]; let typeHtml = `<button class="filter-modal-btn" style="background: #555; color: #fff;" onclick="selectTopFilter('type', '')">ALL TYPES</button>`; types.forEach(t => { typeHtml += `<button class="filter-modal-btn type-${t}" style="background: rgba(255,255,255,0.1);" onclick="selectTopFilter('type', '${t}')">${t}</button>`; }); typeListEl.innerHTML = typeHtml; }
...
window.selectTopFilter = function(filterType, val) {
    if (filterType === 'type') { currentTopType = val; document.getElementById('btn-filter-type').innerHTML = val ? `${val} <span>▼</span>` : `All Types <span>▼</span>`; document.getElementById('btn-filter-type').classList.toggle('active', val !== ''); }
    ...
    closeTopFilterModals(); liveFilterPokemon(tagInput.value.toLowerCase().trim());
};

window.openTopFilterModal = function(modalId) { const overlay = document.getElementById(`${modalId}-overlay`); const modal = document.getElementById(modalId); overlay.style.display = 'block'; setTimeout(() => modal.classList.add('open'), 10); };
window.closeTopFilterModals = function() { ['type-modal', 'regulation-modal'].forEach(modalId => { const modal = document.getElementById(modalId); const overlay = document.getElementById(`${modalId}-overlay`); if (modal) modal.classList.remove('open'); if (overlay) setTimeout(() => overlay.style.display = 'none', 300); }); };
['type-modal-overlay', 'regulation-modal-overlay'].forEach(id => { const el = document.getElementById(id); if (el) el.addEventListener('click', (e) => { if(e.target === el) closeTopFilterModals(); }); });

function buildMasterList() { masterSearchList = []; const types = ["normal", "fire", "water", "grass", "electric", "ice", "fighting", "poison", "ground", "flying", "psychic", "bug", "rock", "ghost", "dragon", "dark", "steel", "fairy"].map(t => ({text: t, tag: t, label: 'Type'})); masterSearchList.push(...types); if(offlineDatabase && offlineDatabase.length > 0) masterSearchList.push(...offlineDatabase.map(p => ({text: p.name.replace(/-/g, ' '), tag: p.name, label: 'Pokémon'}))); if(offlineAbilities && offlineAbilities.length > 0) masterSearchList.push(...offlineAbilities.map(a => ({text: a.name, tag: `ability:${a.name.replace(/ /g, '-')}`, label: 'Ability'}))); if(offlineMoves && offlineMoves.length > 0) masterSearchList.push(...offlineMoves.map(m => ({text: m.name, tag: `move:${m.name.replace(/ /g, '-')}`, label: 'Move'}))); }
tagInput.addEventListener('input', (e) => { const val = e.target.value.toLowerCase().trim(); liveFilterPokemon(val); });
tagInput.addEventListener('keypress', (e) => { if (e.key === 'Enter') { e.preventDefault(); addTag(); } });
function addTag(exactValue = null) { const val = exactValue || tagInput.value.toLowerCase().trim(); if (val !== '' && !activeTags.includes(val)) { activeTags.push(val); tagInput.value = ''; renderTags(); liveFilterPokemon(''); } }
function renderTags() { activeTagsContainer.innerHTML = ''; activeTags.forEach(tag => { let displayTag = tag; let bgColor = "#f44336"; if (tag.startsWith('move:')) { displayTag = '⚔ ' + tag.replace('move:', '').replace(/-/g, ' '); bgColor = "#9c27b0"; } else if (tag.startsWith('ability:')) { displayTag = '✦ ' + tag.replace('ability:', '').replace(/-/g, ' '); bgColor = "#ff9800"; } activeTagsContainer.innerHTML += `<span style="background-color: ${bgColor}; color: white; padding: 5px 12px; border-radius: 20px; display: flex; align-items: center; gap: 8px; font-size: 14px; text-transform: capitalize;">${displayTag}<button onclick="removeTag('${tag}')" style="background: none; border: none; color: white; font-size: 16px; cursor: pointer; padding: 0;">&times;</button></span>`; }); }
window.removeTag = function(tagToRemove) { activeTags = activeTags.filter(t => t !== tagToRemove); renderTags(); liveFilterPokemon(tagInput.value.toLowerCase().trim()); };
...
function liveFilterPokemon(currentInput = '') {
    ...
    if (activeTags.length > 0) { baseList = baseList.filter(pokemon => { return activeTags.every(tag => { if (tag.startsWith('move:')) { const searchMove = tag.replace('move:', ''); const effectiveMoves = getEffectiveMoves(pokemon); return effectiveMoves.includes(searchMove); } if (tag.startsWith('ability:')) { const searchAbility = tag.replace('ability:', ''); return pokemon.abilities.some(a => a.name === searchAbility); } return pokemon.name.toLowerCase().includes(tag) || pokemon.types.some(t => t.toLowerCase() === tag) || pokemon.abilities.some(a => a.name.toLowerCase().includes(tag)); }); }); }
    let liveFiltered = baseList;
    if (currentInput) { liveFiltered = baseList.filter(pokemon => { return pokemon.name.toLowerCase().includes(currentInput) || pokemon.types.some(t => t.toLowerCase().includes(currentInput)) || pokemon.abilities.some(a => a.name.toLowerCase().includes(currentInput)); }); }

    if (currentTopType) liveFiltered = liveFiltered.filter(p => p.types.includes(currentTopType));

    liveFiltered.sort((a, b) => {
        if (currentInput && currentSortMode === 'id') { const aName = a.name.toLowerCase(); const bName = b.name.toLowerCase(); if (aName.startsWith(currentInput) && !bName.startsWith(currentInput)) return -1; if (!aName.startsWith(currentInput) && bName.startsWith(currentInput)) return 1; }
        let valA, valB;
        if (currentSortMode === 'id') { if (a.baseId !== b.baseId) return isDescending ? b.baseId - a.baseId : a.baseId - b.baseId; return a.id - b.id; } 
        else if (currentSortMode === 'name') { return isDescending ? b.name.localeCompare(a.name) : a.name.localeCompare(b.name); } 
        else if (currentSortMode === 'total') { valA = a.hp + a.attack + a.defense + a.spAtk + a.spDef + a.speed; valB = b.hp + b.attack + b.defense + b.spAtk + b.spDef + b.speed; } 
        else { valA = a[currentSortMode]; valB = b[currentSortMode]; }
        return isDescending ? valB - valA : valA - valB;
    });

    currentDisplayedPokemon = liveFiltered; startInfiniteScroll(); 
}

// --- F. THE INFINITE SCROLLER ---
const scrollObserver = new IntersectionObserver((entries) => { if (entries[0].isIntersecting) loadMoreCards(); }, { rootMargin: "200px" }); 
function startInfiniteScroll() { resultsDiv.innerHTML = ''; currentRenderCount = 0; if (currentDisplayedPokemon.length === 0) { resultsDiv.innerHTML = '<p style="text-align:center;">No match.</p>'; return; } loadMoreCards(); }
function loadMoreCards() {
    if (currentRenderCount >= currentDisplayedPokemon.length) return;
    const start = currentRenderCount; const end = Math.min(start + CHUNK_SIZE, currentDisplayedPokemon.length); const chunk = currentDisplayedPokemon.slice(start, end);
    let htmlString = '';
    chunk.forEach(p => {
        const idString = `#${String(p.baseId).padStart(3, '0')}`; 
        const mainType = p.types[0]; const typeChips = p.types.map(t => `<span class="type-chip type-${t}">${t}</span>`).join('');
        let statBadgeHtml = ''; if (currentSortMode === 'total') { const total = p.hp + p.attack + p.defense + p.spAtk + p.spDef + p.speed; statBadgeHtml = `<span class="stat-badge">Total: ${total}</span>`; } else if (['hp', 'attack', 'defense', 'spAtk', 'spDef', 'speed'].includes(currentSortMode)) { const statNames = { hp: 'HP', attack: 'Atk', defense: 'Def', spAtk: 'Sp.A', spDef: 'Sp.D', speed: 'Spe' }; statBadgeHtml = `<span class="stat-badge">${statNames[currentSortMode]}: ${p[currentSortMode]}</span>`; }
        htmlString += `<div class="poke-card type-${mainType}" onclick="handlePokemonClick(${p.id})"><div class="card-info"><div class="card-header"><div class="card-name-group"><span class="card-id">${idString}</span><h3 class="card-name">${p.name.replace(/-/g, ' ')}</h3></div>${statBadgeHtml}</div><div class="card-types">${typeChips}</div></div><div class="card-sprite-container"><img src="https://raw.githubusercontent.com/PokeAPI/sprites/master/sprites/pokemon/${p.id}.png" class="card-sprite" loading="lazy"></div></div>`;
    });
    resultsDiv.insertAdjacentHTML('beforeend', htmlString); currentRenderCount = end; manageSentinel();
}
...
function displayItems(arr) { 
    let filtered = arr;
    if (activeRegulationId) { const activeReg = regulations.find(r => r.id === activeRegulationId); if (activeReg && activeReg.items && activeReg.items.length > 0) filtered = arr.filter(it => activeReg.items.includes(it.name)); }
    if (filtered.length === 0) { itemsResultsDiv.innerHTML = '<p>No items found (or none legal in current Regulation).</p>'; return; } 
    let htmlString = ''; filtered.forEach(it => { htmlString += `<div style="border: 1px solid #444; border-radius: 5px; padding: 10px; margin-bottom: 10px; background-color: #2a2a2a; display: flex; align-items: center; gap: 15px;"><img src="https://raw.githubusercontent.com/PokeAPI/sprites/master/sprites/items/${it.name.replace(/ /g, '-')}.png" style="width: 40px; height: 40px; background: #333; border-radius: 5px; padding: 2px;" loading="lazy" onerror="this.style.display='none'"><div><h4 style="margin: 0; text-transform: capitalize; color:#fff;">${it.name}</h4><p style="margin: 5px 0 0 0; font-size: 14px; color:#ccc;">${it.description}</p></div></div>`; }); itemsResultsDiv.innerHTML = htmlString; 
}
abilitySearch.addEventListener('input', (e) => { const term = e.target.value.toLowerCase(); displayAbilities(offlineAbilities.filter(a => a.name.toLowerCase().includes(term) || a.description.toLowerCase().includes(term))); });
...
function renderRegulationList() {
    let html = '';
    regulations.forEach(r => {
        html += `<div style="background: #2a2a2a; border: 1px solid #555; border-radius: 8px; padding: 15px; margin-bottom: 10px; display: flex; justify-content: space-between; align-items: center;">
...
window.createNewRegulation = function() { editingReg = { id: Date.now().toString(), name: '', pokemon: [], items: [] }; showRegulationEditor(); }
window.editRegulation = function(id) { const r = regulations.find(x => x.id === id); if(r) { editingReg = JSON.parse(JSON.stringify(r)); showRegulationEditor(); } }
window.deleteRegulation = function(id) { if(confirm("Delete this Regulation?")) { regulations = regulations.filter(r => r.id !== id); localStorage.setItem('championsRegulations', JSON.stringify(regulations)); if(activeRegulationId === id) selectTopFilter('regulation', ''); renderRegulationList(); populateTopFilters(); } }
...
window.saveRegulation = function() {
    ...
    if(existingIndex >= 0) regulations[existingIndex] = editingReg; else regulations.push(editingReg);
...
document.getElementById('reg-tab-pkmn').addEventListener('click', (e) => { e.target.classList.add('active'); document.getElementById('reg-tab-items').classList.remove('active'); regTabActive = 'pokemon'; renderRegChips(); });
document.getElementById('reg-tab-items').addEventListener('click', (e) => { e.target.classList.add('active'); document.getElementById('reg-tab-pkmn').classList.remove('active'); regTabActive = 'items'; renderRegChips(); });
...
// --- H2. MASTER LIST MULTI-SELECT LOGIC ---
let masterListFiltered = [];
let masterRenderCount = 0;
const MASTER_CHUNK_SIZE = 60;
const masterListObserver = new IntersectionObserver((entries) => { if (entries[0].isIntersecting) loadMoreMasterCards(); }, { rootMargin: "200px" });
...
function loadMoreMasterCards() {
    ...
    chunk.forEach(item => {
        if (regTabActive === 'pokemon') {
            ...
            html += `<div class="master-card ${isSelected ? 'selected' : ''}" onclick="toggleMasterSelection('${item.name}', this)">
...
window.exportRegulations = function() {
    if (regulations.length === 0) { alert("No custom regulations to export!"); return; }
    const dataStr = "data:text/json;charset=utf-8," + encodeURIComponent(JSON.stringify(regulations));
    const downloadAnchorNode = document.createElement('a');
    downloadAnchorNode.setAttribute("href", dataStr);
    downloadAnchorNode.setAttribute("download", "champions_regulations_backup.json");
...
window.triggerImport = function() {
    document.getElementById('import-reg-file').click();
}

window.importRegulations = function(event) {
        ...
        try {
            ...
            if (Array.isArray(importedData)) {
                ...
                importedData.forEach(importedReg => {
                    ...
                    if (existingIndex >= 0) {
                        regulations[existingIndex] = importedReg;
                    } else {
                        regulations.push(importedReg);
                    }
                });
                ...
                renderRegulationList();
                populateTopFilters();
                alert(`Successfully imported/merged ${importedData.length} regulations!`);
            } else {
                alert("Invalid format: The file does not contain a valid regulations array.");
            }
        } catch (err) {
...
window.getEffectiveMoves = function(pokemon) {
    ...
    if (offlineOverrides && offlineOverrides[pokemon.id]) {
        ...
        added.forEach(m => { if(!moves.includes(m)) moves.push(m); });
    }
...
window.openLearnsetModal = function() {
    ...
    if (!offlineOverrides[editingLearnsetTarget]) offlineOverrides[editingLearnsetTarget] = { added: [], removed: [] };
...
function renderLearnsetCurrentMoves() {
    ...
    effectiveMoves.forEach(m => {
        ...
        container.innerHTML += `<div class="override-move-row">
...
window.removeMoveFromLearnset = function(moveName) {
    ...
    if (isBaseMove) {
        if(!offlineOverrides[editingLearnsetTarget].removed) offlineOverrides[editingLearnsetTarget].removed = [];
...
document.querySelectorAll('.team-slot').forEach((slot, index) => { 
    slot.addEventListener('click', () => { 
        if (currentTeam[index] === null) { activeSlotIndex = index; switchView('tab-pokemon-btn'); tagInput.placeholder = `Select for Slot ${index + 1}...`; window.scrollTo({ top: 0, behavior: 'smooth' }); tagInput.focus(); } 
        else { openTeamEditModal(index); }
    }); 
});
...
window.selectPokemonForTeam = function(pokemonId) { 
    ...
    if (selectedPkmn) { 
        currentTeam[activeSlotIndex] = { data: selectedPkmn, ability: selectedPkmn.abilities.length > 0 ? selectedPkmn.abilities[0].name : 'None', item: 'None', moves: ['None', 'None', 'None', 'None'] }; 
        activeSlotIndex = null; tagInput.placeholder = "Quick search... (Press Enter to lock tag)"; tagInput.value = ''; liveFilterPokemon(''); renderTeamGrid(); switchView('tab-teambuilder-btn'); 
    } 
};

function renderTeamGrid() {
    currentTeam.forEach((member, index) => {
        const slotEl = document.getElementById(`slot-${index + 1}`); if(!slotEl) return;
        if (member) {
            const mainType = member.data.types[0]; slotEl.style.borderStyle = 'solid'; slotEl.style.borderColor = '#888'; slotEl.style.backgroundColor = '#333';
            slotEl.innerHTML = `<img src="https://raw.githubusercontent.com/PokeAPI/sprites/master/sprites/pokemon/${member.data.id}.png" style="width: 80px; height: 80px;" loading="lazy"><span style="color: #fff; text-transform: capitalize; font-size: 14px;">${member.data.name.replace(/-/g, ' ')}</span><span class="type-chip type-${mainType}" style="margin-top: 5px; font-size: 9px;">${mainType}</span>`;
        } else { slotEl.style.borderStyle = 'dashed'; slotEl.style.borderColor = '#555'; slotEl.style.backgroundColor = '#2a2a2a'; slotEl.innerHTML = `<span class="slot-icon">⊕</span><span>Add Member</span>`; }
    });
}

let editingSlotIndex = null; let editingMoveIndex = null;
const teamEditOverlay = document.getElementById('team-edit-overlay'); const teamEditModal = document.getElementById('team-edit-modal'); const editSprite = document.getElementById('edit-sprite'); const editName = document.getElementById('edit-name'); const editAbilitySelect = document.getElementById('edit-ability'); const editItemSpan = document.getElementById('edit-item'); const editMoveBtns = [document.getElementById('edit-move-1'), document.getElementById('edit-move-2'), document.getElementById('edit-move-3'), document.getElementById('edit-move-4')];

window.openTeamEditModal = function(index) {
    editingSlotIndex = index; const member = currentTeam[index];
    editSprite.src = `https://raw.githubusercontent.com/PokeAPI/sprites/master/sprites/pokemon/${member.data.id}.png`; editName.innerText = member.data.name.replace(/-/g, ' ');
    ...
    editMoveBtns.forEach((btn, i) => { btn.innerText = member.moves[i].replace(/-/g, ' '); if (member.moves[i] === 'None') { btn.style.color = '#888'; btn.style.borderColor = '#444'; } else { btn.style.color = '#fff'; btn.style.borderColor = '#555'; } });
    teamEditOverlay.style.display = 'block'; setTimeout(() => teamEditModal.classList.add('open'), 10);
};

window.showAbilityInfo = function() { const abName = editAbilitySelect.value; const abData = offlineAbilities.find(a => a.name === abName); if (abData) { const safeDesc = abData.description.replace(/`/g, "'").replace(/\\/g, ""); alert(abData.name.toUpperCase() + '\n\n' + safeDesc); } }
window.showItemInfo = function() { if (editingSlotIndex === null) return; const itemName = currentTeam[editingSlotIndex].item; if (itemName === 'None') { alert("No item selected."); return; } const searchName = itemName.replace(/ /g, '-').toLowerCase(); const itemData = offlineItems.find(i => i.name.toLowerCase().replace(/ /g, '-') === searchName); if (itemData) { const safeDesc = itemData.description.replace(/`/g, "'").replace(/\\/g, ""); alert(itemData.name.toUpperCase() + '\n\n' + safeDesc); } }

window.closeTeamEditModal = function() { if (editingSlotIndex !== null && currentTeam[editingSlotIndex]) { currentTeam[editingSlotIndex].ability = editAbilitySelect.value; } teamEditModal.classList.remove('open'); setTimeout(() => teamEditOverlay.style.display = 'none', 300); };
...
if (itemSelectOverlay) itemSelectOverlay.addEventListener('click', (e) => { if(e.target === itemSelectOverlay) closeItemSelectModal(); });
...
function renderItemSelectList(searchTerm) {
    ...
    filtered.forEach(it => { const safeDesc = it.description.replace(/`/g, "'").replace(/\\/g, ""); html += `<div style="display: flex; align-items: center; gap: 15px; padding: 10px; border-bottom: 1px solid #444;"><div onclick="selectItemForTeam('${it.name}')" style="display: flex; align-items: center; gap: 15px; flex-grow: 1; cursor: pointer;"><img src="https://raw.githubusercontent.com/PokeAPI/sprites/master/sprites/items/${it.name.replace(/ /g, '-')}.png" style="width: 30px; height: 30px;" loading="lazy" onerror="this.style.display='none'"><div><div style="font-weight: bold; text-transform: capitalize; color: #fff;">${it.name.replace(/-/g, ' ')}</div></div></div><button onclick="alert(\`${safeDesc}\`)" style="background: #444; border: 1px solid #555; color: #fff; width: 35px; height: 35px; border-radius: 50%; font-weight: bold; cursor: pointer; flex-shrink: 0;">ⓘ</button></div>`; }); itemSelectList.innerHTML = html;
}
...
if (moveSelectOverlay) moveSelectOverlay.addEventListener('click', (e) => { if(e.target === moveSelectOverlay) closeMoveSelectModal(); });
...
function renderMoveSelectList(searchTerm) {
    ...
    validMoves.forEach(m => { let dmgClassColor = 'dmg-status'; if(m.damage_class === 'physical') dmgClassColor = 'dmg-physical'; if(m.damage_class === 'special') dmgClassColor = 'dmg-special'; const safeDesc = m.description.replace(/`/g, "'").replace(/\\/g, ""); html += `<div class="detail-move-row" style="margin: 5px 10px; display: flex; align-items: center; gap: 10px;"><div style="flex-grow: 1;" onclick="selectMoveForTeam('${m.name}')"><div class="detail-move-header"><div class="detail-move-name">${m.name}</div><div class="detail-move-stat">${m.power}</div><div class="detail-move-stat">${m.accuracy}</div><div class="detail-move-stat" style="text-align: right;">${m.pp}</div></div><div class="detail-move-tags"><span class="type-chip type-${m.type}" style="margin: 0;">${m.type}</span><span class="dmg-class ${dmgClassColor}">${m.damage_class}</span></div></div><button onclick="alert(\`${safeDesc}\`)" style="background: #444; border: 1px solid #555; color: #fff; width: 40px; height: 40px; border-radius: 50%; font-weight: bold; cursor: pointer; flex-shrink: 0;">ⓘ</button></div>`; }); moveSelectList.innerHTML = html;
}
window.selectMoveForTeam = function(moveName) { if (editingSlotIndex !== null && editingMoveIndex !== null) { currentTeam[editingSlotIndex].moves[editingMoveIndex] = moveName; const btn = editMoveBtns[editingMoveIndex]; btn.innerText = moveName.replace(/-/g, ' '); if (moveName === 'None') { btn.style.color = '#888'; btn.style.borderColor = '#444'; } else { btn.style.color = '#fff'; btn.style.borderColor = '#555'; } closeMoveSelectModal(); } }

window.saveCurrentTeam = function() {
    const isEmpty = currentTeam.every(m => m === null);
    if (isEmpty) { alert("Cannot save an empty team!"); return; }
...
function renderTeamManagerList() {
    let html = '';
    if (savedTeams.length === 0) { html = '<p style="text-align:center; color:#888; margin-top: 30px;">No teams saved yet.</p>'; } else {
        savedTeams.forEach((teamObj, idx) => {
            let spritesHtml = teamObj.team.map(m => m ? `<img src="https://raw.githubusercontent.com/PokeAPI/sprites/master/sprites/pokemon/${m.data.id}.png" style="width:35px;height:35px; margin:-5px; filter: drop-shadow(1px 1px 2px rgba(0,0,0,0.8));">` : `<span style="width:25px; height:35px; display:inline-block;"></span>`).join('');
            html += `<div style="background: #2a2a2a; border: 1px solid #555; border-radius: 8px; padding: 10px; margin-bottom: 10px; display: flex; justify-content: space-between; align-items: center;"><div style="flex-grow: 1; cursor: pointer; padding-right: 10px;" onclick="loadSavedTeam(${idx})"><div style="font-weight: bold; color: #fff; margin-bottom: 8px; font-size: 16px;">${teamObj.name}</div><div style="display:flex;">${spritesHtml}</div></div><button onclick="deleteSavedTeam(${idx})" style="background: #f44336; color: #fff; border: none; padding: 10px 15px; border-radius: 8px; font-weight: bold; cursor: pointer;">Delete</button></div>`;
        });
    }
...
const detailHeader = document.getElementById('detail-header'); const detailName = document.getElementById('detail-name'); const detailId = document.getElementById('detail-id'); const detailTypes = document.getElementById('detail-types'); const detailSprite = document.getElementById('detail-sprite');
const detailStats = document.getElementById('detail-stats'); const detailAbilities = document.getElementById('detail-abilities');
const detailTabInfo = document.getElementById('detail-tab-info'); const detailTabMoves = document.getElementById('detail-tab-moves'); const detailMovesList = document.getElementById('detail-moves-list');
const detailNavInfo = document.getElementById('detail-nav-info'); const detailNavMoves = document.getElementById('detail-nav-moves');
const detailFormsContainer = document.getElementById('detail-forms-container'); const detailFormsList = document.getElementById('detail-forms-list');
const detailEvoContainer = document.getElementById('detail-evo-container'); const detailEvoList = document.getElementById('detail-evo-list');

window.openDetailedView = function(pokemonId) {
    const p = offlineDatabase.find(x => x.id === pokemonId); if (!p) return;
    window.currentDetailedPokemon = p; 

    detailTabInfo.style.display = 'block'; detailTabMoves.style.display = 'none'; detailNavInfo.classList.add('active'); detailNavMoves.classList.remove('active');

    const mainType = p.types[0]; detailHeader.className = `type-${mainType}`; detailName.innerText = p.name.replace(/-/g, ' '); 
    detailId.innerText = `#${String(p.baseId).padStart(3, '0')}`; 
    detailSprite.src = `https://raw.githubusercontent.com/PokeAPI/sprites/master/sprites/pokemon/${p.id}.png`; detailTypes.innerHTML = p.types.map(t => `<span class="type-chip type-${t}" style="background: rgba(255,255,255,0.3); border: 1px solid rgba(0,0,0,0.2);">${t}</span>`).join('');

    const maxStat = 255; const statsList = [ { label: 'HP', val: p.hp }, { label: 'Attack', val: p.attack }, { label: 'Defense', val: p.defense }, { label: 'Sp. Atk', val: p.spAtk }, { label: 'Sp. Def', val: p.spDef }, { label: 'Speed', val: p.speed } ];
    let statsHtml = ''; let totalStats = 0; statsList.forEach(s => { totalStats += s.val; let barColor = '#f44336'; if (s.val >= 60) barColor = '#ff9800'; if (s.val >= 90) barColor = '#4caf50'; if (s.val >= 120) barColor = '#00bcd4'; const pct = (s.val / maxStat) * 100; statsHtml += `<div class="stat-row"><div class="stat-label">${s.label}</div><div class="stat-val">${s.val}</div><div class="stat-bar-bg"><div class="stat-bar-fill" style="width: 0%; background: ${barColor};" data-target="${pct}%"></div></div></div>`; }); statsHtml += `<div style="text-align: center; margin-top: 15px; font-weight: bold; color: #888;">TOTAL: ${totalStats}</div>`; detailStats.innerHTML = statsHtml;

    let abilityHtml = ''; p.abilities.forEach(ab => { const fullAb = offlineAbilities.find(x => x.name.toLowerCase() === ab.name.replace(/-/g, ' ')); const desc = fullAb ? fullAb.description : "Description not loaded."; abilityHtml += `<div style="margin-bottom: 10px; background: #333; padding: 10px; border-radius: 8px;"><div style="font-weight: bold; text-transform: capitalize; margin-bottom: 5px;">${ab.name.replace(/-/g, ' ')} ${ab.isHidden ? '<span style="font-size:10px; background:#9c27b0; padding:2px 6px; border-radius:4px; margin-left:5px;">HIDDEN</span>' : ''}</div><div style="font-size: 13px; color: #bbb;">${desc}</div></div>`; }); detailAbilities.innerHTML = abilityHtml;

    const baseSpecies = offlineDatabase.find(x => x.id === p.baseId);
    let foundEvoHtml = '';
    if (baseSpecies && offlineEvolutions.length > 0) {
        const chain = offlineEvolutions.find(c => c.some(node => node.name === baseSpecies.speciesName));
        if (chain && chain.length > 1) {
            const chainHtml = chain.map((node) => {
                ...
                return `<div class="evo-card ${isCurrentBase}" onclick="openDetailedView(${evoPkmn.id})"><img class="evo-sprite" src="https://raw.githubusercontent.com/PokeAPI/sprites/master/sprites/pokemon/${evoPkmn.id}.png"><div class="evo-name">${evoPkmn.name.replace(/-/g, ' ')}</div><div style="font-size:9px; color:#aaa; margin-top:3px; text-transform:capitalize; text-align:center;">${detailText}</div></div>`;
            }).join('');
            if (chainHtml) foundEvoHtml = chainHtml;
        }
    }
    if (foundEvoHtml) { detailEvoContainer.style.display = 'block'; detailEvoList.innerHTML = foundEvoHtml; } else { detailEvoContainer.style.display = 'none'; }

    const altForms = offlineDatabase.filter(x => x.baseId === p.baseId);
    if (altForms.length > 1) {
        detailFormsContainer.style.display = 'block';
        detailFormsList.innerHTML = altForms.map(form => {
            let label = "Base"; if (form.formType === 'mega') label = 'Mega'; else if (form.formType === 'gmax') label = 'G-Max'; else if (form.formType === 'regional') label = 'Regional'; else if (form.id !== p.baseId) label = 'Alt Form';
            const isCurrent = form.id === p.id ? 'current' : '';
            return `<div class="evo-card ${isCurrent}" onclick="openDetailedView(${form.id})"><img class="evo-sprite" src="https://raw.githubusercontent.com/PokeAPI/sprites/master/sprites/pokemon/${form.id}.png"><div class="evo-name">${label}</div></div>`;
        }).join('');
    } else { detailFormsContainer.style.display = 'none'; }
...
window.renderPokemonMoves = function(p) {
    ...
    effectiveMoves.forEach(moveName => {
        const moveData = offlineMoves.find(m => m.name.toLowerCase() === moveName.replace(/-/g, ' '));
        if(moveData) {
            ...
            movesHtml += `<div class="detail-move-row"><div class="detail-move-header"><div class="detail-move-name" style="flex: 2;">${moveData.name}</div><div class="detail-move-stat">${moveData.power}</div><div class="detail-move-stat">${moveData.accuracy}</div><div class="detail-move-stat" style="text-align: right;">${moveData.pp}</div></div><div class="detail-move-tags"><span class="type-chip type-${moveData.type}" style="margin: 0;">${moveData.type}</span><span class="dmg-class ${dmgClassColor}">${moveData.damage_class}</span></div></div>`;
        }
    });
    
    if (effectiveMoves.length === 0) { movesHtml = `<p style="text-align:center; color:#888; margin-top:30px;">No moves found. Click "Edit Base Learnset" to add some.</p>`; }
    detailMovesList.innerHTML = movesHtml;
}

window.closeDetailedView = function() { viewDetails.style.display = 'none'; };
if (detailNavInfo) { detailNavInfo.addEventListener('click', () => { detailTabInfo.style.display = 'block'; detailTabMoves.style.display = 'none'; detailNavInfo.classList.add('active'); detailNavMoves.classList.remove('active'); window.scrollTo({ top: 0 }); }); }
if (detailNavMoves) { detailNavMoves.addEventListener('click', () => { detailTabInfo.style.display = 'none'; detailTabMoves.style.display = 'block'; detailNavInfo.classList.remove('active'); detailNavMoves.classList.add('active'); window.scrollTo({ top: 0 }); }); }

// --- L. FAB, SORT, MOVE, ABILITY, & TAG MENU LOGIC ---
const fabToggle = document.getElementById('fab-toggle'); const fabMenu = document.getElementById('fab-menu'); const fabBtnSearch = document.getElementById('fab-btn-search'); const fabBtnSort = document.getElementById('fab-btn-sort'); const fabBtnMove = document.getElementById('fab-btn-move'); const fabBtnAbility = document.getElementById('fab-btn-ability');
const sortModalOverlay = document.getElementById('sort-modal-overlay'); const sortModal = document.getElementById('sort-modal'); const closeSortModalBtn = document.getElementById('close-sort-modal'); const sortButtons = document.querySelectorAll('.sort-btn'); const btnAsc = document.getElementById('sort-asc'); const btnDesc = document.getElementById('sort-desc');
const moveModalOverlay = document.getElementById('move-modal-overlay'); const moveModal = document.getElementById('move-modal'); const closeMoveModalBtn = document.getElementById('close-move-modal'); const moveFilterInput = document.getElementById('move-filter-input'); const moveFilterSuggestions = document.getElementById('move-filter-suggestions');
const abilityModalOverlay = document.getElementById('ability-modal-overlay'); const abilityModal = document.getElementById('ability-modal'); const closeAbilityModalBtn = document.getElementById('close-ability-modal'); const abilityFilterInput = document.getElementById('ability-filter-input'); const abilityFilterSuggestions = document.getElementById('ability-filter-suggestions');
const tagModalOverlay = document.getElementById('tag-modal-overlay'); const tagModal = document.getElementById('tag-modal'); const closeTagModalBtn = document.getElementById('close-tag-modal'); const tagFilterInput = document.getElementById('tag-filter-input'); const tagFilterSuggestions = document.getElementById('tag-filter-suggestions');

if (fabToggle) { fabToggle.addEventListener('click', () => { fabToggle.classList.toggle('active'); fabMenu.classList.toggle('active'); }); }
if (fabBtnSearch) { fabBtnSearch.addEventListener('click', () => { fabToggle.classList.remove('active'); fabMenu.classList.remove('active'); tagModalOverlay.style.display = 'block'; setTimeout(() => tagModal.classList.add('open'), 10); tagFilterInput.focus(); }); }
if (fabBtnSort) { fabBtnSort.addEventListener('click', () => { fabToggle.classList.remove('active'); fabMenu.classList.remove('active'); sortModalOverlay.style.display = 'block'; setTimeout(() => sortModal.classList.add('open'), 10); }); }
function closeSortModal() { sortModal.classList.remove('open'); setTimeout(() => sortModalOverlay.style.display = 'none', 300); }
if (closeSortModalBtn) closeSortModalBtn.addEventListener('click', closeSortModal);
if (sortModalOverlay) sortModalOverlay.addEventListener('click', (e) => { if (e.target === sortModalOverlay) closeSortModal(); });
sortButtons.forEach(btn => { btn.addEventListener('click', (e) => { sortButtons.forEach(b => b.classList.remove('active')); e.target.classList.add('active'); currentSortMode = e.target.getAttribute('data-sort'); if(['id', 'name'].includes(currentSortMode)) { isDescending = false; btnAsc.classList.add('active'); btnDesc.classList.remove('active'); } else { isDescending = true; btnDesc.classList.add('active'); btnAsc.classList.remove('active'); } closeSortModal(); liveFilterPokemon(tagInput.value.toLowerCase().trim()); window.scrollTo({ top: 0, behavior: 'smooth' }); }); });
if (btnAsc && btnDesc) { btnAsc.addEventListener('click', () => { isDescending = false; btnAsc.classList.add('active'); btnDesc.classList.remove('active'); closeSortModal(); liveFilterPokemon(tagInput.value.toLowerCase().trim()); window.scrollTo({ top: 0, behavior: 'smooth' }); }); btnDesc.addEventListener('click', () => { isDescending = true; btnDesc.classList.add('active'); btnAsc.classList.remove('active'); closeSortModal(); liveFilterPokemon(tagInput.value.toLowerCase().trim()); window.scrollTo({ top: 0, behavior: 'smooth' }); }); }

if (fabBtnMove) { fabBtnMove.addEventListener('click', () => { fabToggle.classList.remove('active'); fabMenu.classList.remove('active'); moveModalOverlay.style.display = 'block'; setTimeout(() => moveModal.classList.add('open'), 10); moveFilterInput.focus(); }); }
function closeMoveModal() { moveModal.classList.remove('open'); setTimeout(() => moveModalOverlay.style.display = 'none', 300); moveFilterInput.value = ''; moveFilterSuggestions.style.display = 'none'; }
if (closeMoveModalBtn) closeMoveModalBtn.addEventListener('click', closeMoveModal);
if (moveModalOverlay) moveModalOverlay.addEventListener('click', (e) => { if (e.target === moveModalOverlay) closeMoveModal(); });
moveFilterInput.addEventListener('input', (e) => { const val = e.target.value.toLowerCase().trim(); moveFilterSuggestions.innerHTML = ''; if (val.length === 0) { moveFilterSuggestions.style.display = 'none'; return; } const matches = offlineMoves.filter(m => m.name.toLowerCase().includes(val)).slice(0, 20); if (matches.length > 0) { moveFilterSuggestions.style.display = 'block'; matches.forEach(match => { const div = document.createElement('div'); div.style.cssText = 'padding: 12px 10px; border-bottom: 1px solid #555; cursor: pointer; text-transform: capitalize; color: #fff;'; div.innerText = match.name; div.onclick = () => { addTag(`move:${match.name.replace(/ /g, '-')}`); closeMoveModal(); }; moveFilterSuggestions.appendChild(div); }); } else { moveFilterSuggestions.style.display = 'none'; } });

if (fabBtnAbility) { fabBtnAbility.addEventListener('click', () => { fabToggle.classList.remove('active'); fabMenu.classList.remove('active'); abilityModalOverlay.style.display = 'block'; setTimeout(() => abilityModal.classList.add('open'), 10); abilityFilterInput.focus(); }); }
function closeAbilityModal() { abilityModal.classList.remove('open'); setTimeout(() => abilityModalOverlay.style.display = 'none', 300); abilityFilterInput.value = ''; abilityFilterSuggestions.style.display = 'none'; }
if (closeAbilityModalBtn) closeAbilityModalBtn.addEventListener('click', closeAbilityModal);
if (abilityModalOverlay) abilityModalOverlay.addEventListener('click', (e) => { if (e.target === abilityModalOverlay) closeAbilityModal(); });
abilityFilterInput.addEventListener('input', (e) => { const val = e.target.value.toLowerCase().trim(); abilityFilterSuggestions.innerHTML = ''; if (val.length === 0) { abilityFilterSuggestions.style.display = 'none'; return; } const matches = offlineAbilities.filter(a => a.name.toLowerCase().includes(val)).slice(0, 20); if (matches.length > 0) { abilityFilterSuggestions.style.display = 'block'; matches.forEach(match => { const div = document.createElement('div'); div.style.cssText = 'padding: 12px 10px; border-bottom: 1px solid #555; cursor: pointer; text-transform: capitalize; color: #fff;'; div.innerText = match.name; div.onclick = () => { addTag(`ability:${match.name.replace(/ /g, '-')}`); closeAbilityModal(); }; abilityFilterSuggestions.appendChild(div); }); } else { abilityFilterSuggestions.style.display = 'none'; } });

if (fabBtnSearch) { fabBtnSearch.addEventListener('click', () => { fabToggle.classList.remove('active'); fabMenu.classList.remove('active'); tagModalOverlay.style.display = 'block'; setTimeout(() => tagModal.classList.add('open'), 10); tagFilterInput.focus(); }); }
function closeTagModal() { tagModal.classList.remove('open'); setTimeout(() => tagModalOverlay.style.display = 'none', 300); tagFilterInput.value = ''; tagFilterSuggestions.style.display = 'none'; }
...
async function initializeApp() {
    if (syncAllBtn) syncAllBtn.addEventListener('click', syncAllData);
    await loadOfflineData(); await loadOfflineAbilities(); await loadOfflineMoves(); await loadOfflineItems();
    await loadOfflineEvolutions(); await loadOfflineOverrides();
}
initializeApp();
