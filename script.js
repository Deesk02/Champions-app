...
window.renderPokemonMoves = function(p) {
    ...
    moveList.forEach(moveName => {
        ...
        if(moveData) {
            ...
            movesHtml += `<div class="detail-move-row"><div class="detail-move-header"><div class="detail-move-name" style="flex: 2; display: flex; align-items: center;">${moveData.name} ${customTag}</div><div class="detail-move-stat">${moveData.power}</div><div class="detail-move-stat">${moveData.accuracy}</div><div class="detail-move-stat" style="text-align: right;">${moveData.pp}</div></div><div class="detail-move-tags"><span class="type-chip type-${moveData.type}" style="margin: 0;">${moveData.type}</span><span class="dmg-class ${dmgClassColor}">${moveData.damage_class}</span></div></div>`;
        }
    });
